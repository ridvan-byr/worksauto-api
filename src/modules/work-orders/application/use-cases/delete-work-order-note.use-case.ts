...
export class DeleteWorkOrderNoteUseCase {
  ...
  async execute(
    ...
    const isAuthor = note.authorId ? note.authorId === user.id : note.authorName === fullName;
    const isManager = user.role === UserRole.OWNER || user.role === UserRole.SERVICE_MANAGER;

    if (!isAuthor && !isManager) {
      throw new ForbiddenException('Bu mesajı silme yetkiniz bulunmamaktadır. Yalnızca kendi mesajlarınızı veya yönetici iseniz silebilirsiniz.');
    }
    ...
    try {
      await this.auditService.log({
        ...
        changesBefore: { noteId, authorName: note.authorName, text: note.text, deletedBy: fullName, isManagerAction: !isAuthor && isManager },
      });
    } catch {
      // Audit non-fatal
    }
