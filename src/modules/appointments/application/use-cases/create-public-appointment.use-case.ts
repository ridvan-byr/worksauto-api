...
export class CreatePublicAppointmentUseCase {
  ...
  async execute(
    slug: string,
    dto: CreatePublicAppointmentInput,
  ): Promise<AppointmentEntity> {
    const tenant = await this.appointmentRepository.findTenantBySlug(slug);
    if (!tenant) {
      throw new NotFoundException(
        'İlgili işletme bulunamadı veya online randevu kapalı.',
      );
    }
    ...
    if (isNaN(start.getTime()) || isNaN(end.getTime()) || end <= start) {
      throw new BadRequestException(
        'Geçersiz randevu saat aralığı. Bitiş saati başlangıçtan sonra olmalıdır.',
      );
    }
    ...
    const onlineBays = await this.appointmentRepository.findActiveOnlineBays(
      tenant.id,
    );
    const bays =
      onlineBays.length > 0
        ? onlineBays
        : ['Lift 1 (Hızlı Kabul)', 'Lift 2 (Mekanik)', 'Kabul Alanı'];
    ...
    if (!assignedLift) {
      throw new ConflictException(
        'Seçilen saat aralığında tüm servis kabul alanları doludur. Lütfen farklı bir saat dilimi seçiniz.',
      );
    }

    const customer =
      await this.appointmentRepository.findOrCreateCustomerForPublic(
        tenant.id,
        dto.customerName,
        dto.customerPhone,
      );
    ...
    const vehicle =
      await this.appointmentRepository.findOrCreateVehicleForPublic(
        tenant.id,
        customer.id,
        dto.plate,
        dto.brandModel,
