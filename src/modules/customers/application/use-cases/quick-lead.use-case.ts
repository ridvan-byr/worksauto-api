...
import { ICustomerRepository, CUSTOMER_REPOSITORY, QuickLeadInput } from '../../domain/customer.repository.interface';
...
export class QuickLeadUseCase {
  ...
  async execute(tenantId: string, dto: QuickLeadInput) {
    if (!dto.firstName || !dto.phone || !dto.plate) {
      throw new BadRequestException('Müşteri adı, telefon ve araç plakası zorunludur.');
    }
