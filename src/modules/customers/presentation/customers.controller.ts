...
import { CreateCustomerDto, QuickLeadDto, BatchImportRequestDto } from '../dto/customer.dto';
...
export class CustomersController {
  ...
  @Get(':id/consents')
  @Roles(UserRole.OWNER, UserRole.SERVICE_MANAGER, UserRole.CASHIER, UserRole.TECHNICIAN)
  @ApiOperation({ summary: 'Müşterinin KVKK ve ticari SMS onay durumunu ve tarihçesini döner' })
  ...
  @Roles(UserRole.OWNER, UserRole.SERVICE_MANAGER, UserRole.CASHIER)
  @ApiOperation({ summary: 'Müşteriye KVKK ve İYS dijital onay bağlantısı üretir ve SMS gönderir' })
  ...
  @Roles(UserRole.OWNER, UserRole.SERVICE_MANAGER, UserRole.CASHIER)
  @ApiOperation({ summary: 'Fiziksel form veya tezgah üstü onayını doğrudan işler' })
  recordDirectConsent(
    @CurrentTenant() tenantId: string,
    @Param('id') id: string,
    ...
    return this.manageConsentUseCase.recordDirectConsent(tenantId, id, dto, clientIp, userAgent);
  }
}
